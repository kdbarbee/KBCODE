...
A = 100; % INPUT: Define y-int value for model linear fnc
k = 100; % INPUT: Define slope for model linear fnc

ysin = A + k*xsin; % Define model linear fnc for non-replicate data set
yrep = A + k*xrep; % Define model linear fnc for replicate data set

xfin = round(linspace(lwr_range,upr_range,100)); % Create fine x-data vector for smooth curve of model linear
yfin = A + (k*xfin); % Define model linear fnc for fine x data set
...
for nb=1:nb_int;
    ...
    for p=1:sims;
        ...
        for i=1:trials; % Begin loop for generating noisy data and curve fitting to each noisy data set
            ...
            yrep_obj = @(b,x) b(1) + (b(2).*x);             % Objective function for replicate data
            ...
            ysin_obj = @(c,x) c(1) + (c(2).*x);             % Objective function for single data points
        ...
        yrep_fit = Arep_fit + (krep_fit*xfin); % Generate y values for linear fnc from fit parameters (replicate)
        ysin_fit = Asin_fit + (ksin_fit*xfin); % Generate y values for linear fnc from fit parameters (singles)
