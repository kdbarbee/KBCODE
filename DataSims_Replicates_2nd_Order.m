...
xvals = [5,20,35,50]; % INPUT: Defines the number of unique inputs as well as the range
...
A = 1; % INPUT: Define y-int value for model fnc
k = 0.1; % INPUT: Define slope for model fnc

% y = 1/(A+kx)
ysin = 1./(A + k*xsin); % Define model  fnc for non-replicate data set
yrep = 1./(A + k*xrep); % Define model  fnc for replicate data set

xfin = round(linspace(lwr_range,upr_range,100)); % Create fine x-data vector for smooth curve of model function
yfin = 1./(A + k*xfin); % Define model  fnc for fine x data set
...
for nb=1:nb_int;
    ...
    for p=1:sims;
        ...
        for i=1:trials; % Begin loop for generating noisy data and curve fitting to each noisy data set
            ...
            yrep_obj = @(b,x) 1./(b(1) + b(2).*x);             % Objective function for replicate data
            ...
            ysin_obj = @(c,x) 1./(c(1) + c(2).*x);             % Objective function for single data points
        ...
        yrep_fit = 1./(Arep_fit + krep_fit*xfin); % Generate y values for  fnc from fit parameters (replicate)
        ysin_fit = 1./(Asin_fit + ksin_fit*xfin); % Generate y values for  fnc from fit parameters (singles)
