...
A = 100; % INPUT: Define initial value for model logarithmic fnc
k = 1; % INPUT: Define log constant for model logarithmic fnc

ysin = A*log(k*xsin); % Define model logarithmic fnc for non-replicate data set
yrep = A*log(k*xrep); % Define model logarithmic fnc for replicate data set

xfin = round(linspace(lwr_range,upr_range,100)); % Create fine x-data vector for smooth curve of model logarithmic
yfin = A*log(k*xfin); % Define model logarithmic fnc for fine x data set
...
trials=25; % INPUT: Define number of trials, where each trial generates a new noisy data set for subsequent curve fitting
% % % % % % % % % % % % % % % % % % % % 
...
for nb=1:nb_int;
    ...
    for p=1:sims;
        ...
        for i=1:trials; % Begin loop for generating noisy data and curve fitting to each noisy data set
            ...
            ysin_noisy=ysin.*noise; % Apply noise to non-replicate data
            ...
            % Fitting data to a logarithmic function:
            yrep_obj = @(b,x) b(1).*log(b(2).*x);             % Objective function for replicate data
            ...
            ysin_obj = @(c,x) c(1).*log(c(2).*x);             % Objective function for single data points
...
%             end       
        ...
        yrep_fit = Arep_fit*log(krep_fit*xfin); % Generate y values for logarithmic fnc from fit parameters (replicate)
        ysin_fit = Asin_fit*log(ksin_fit*xfin); % Generate y values for logarithmic fnc from fit parameters (singles)
